...
*/
#define EXAMPLE_WIFI_SSID "Nanolever"
#define EXAMPLE_WIFI_PASS "V&enhBgd"
...
void aws_iot_task(void *param) {
    uint8_t paramPassed = *((uint8_t*) param);
    char cPayload[100];
    ...
    while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)) {
        ...
        vTaskDelay(1000 / portTICK_RATE_MS);
        sprintf(cPayload, "%s : %d ", "hello from ESP32 (QOS0)", paramPassed);
        paramsQOS0.payloadLen = strlen(cPayload);
        rc = aws_iot_mqtt_publish(&client, TOPIC, TOPIC_LEN, &paramsQOS0);

        sprintf(cPayload, '{ "%s" : "%d" }', "Temperature", i++);
...
void app_main()
{
    ...
    initialise_wifi();
    xTaskCreatePinnedToCore(&aws_iot_task, "aws_iot_task", 9216, &ucParameterToPass, 5, NULL, 1);
}
